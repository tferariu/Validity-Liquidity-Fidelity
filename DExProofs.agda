...
getS : ScriptContext -> Label -> State
getS ctx l = record { label = l
                    ; context = record { value = (inputVal ctx) 
                                       ; payVal = record { amount = -1 ; currency = 0 }
                                       ; payTo = 0
                                       ; payDat = Payment 0
                                       ; buyVal = record { amount = -1 ; currency = 0 }
                                       ; buyTo = 0
                                       ; buyDat = Payment 0
                                       ; tsig = 0
                                       ; self = 0 }
...
getS' : ScriptContext -> Label -> Input -> State
getS' ctx l i = record { label = newLabel ctx 
                       ; context = record { value = newValue ctx
                                          ; payVal = getPayOutVal l ctx
                                          ; payTo = getPayOutAdr l ctx
                                          ; payDat = getPayOutDat l ctx
                                          ; buyVal = getBuyOutVal l i ctx
                                          ; buyTo = getBuyOutAdr l i ctx
                                          ; buyDat = getBuyOutDat l i ctx
                                          ; tsig = signature ctx
                                          ; self = getSelf ctx }
...
transitionImpliesValidator : ∀ (par : Params) (l : Label) (i : Input) (ctx : ScriptContext)
                           -> (pf : par ⊢ getS ctx l ~[ i ]~> getS' ctx l i)
...
Info = (Params × Label × Input × ScriptContext)

toF : Info -> Bool
toF (par , dat , i , ctx) = agdaValidator par dat i ctx

toR : Info -> Set
toR (par , dat , i , ctx) =  par ⊢ getS ctx dat ~[ i ]~> getS' ctx dat i

rewriteTo : ∀ {a : Info} -> toF a ≡ true -> toR a
rewriteTo {par , dat , i , ctx} pf = validatorImpliesTransition par dat i ctx pf

rewriteFrom : ∀ {a : Info} ->  toR a -> toF a ≡ true
rewriteFrom {par , dat , i , ctx} pf = transitionImpliesValidator par dat i ctx pf 

funcEquiv : toF ≈ toR 
funcEquiv = record { to = rewriteTo ; from = rewriteFrom }
